...
public class Solution {
    ...
    {
        final AtomicInteger poolId = new AtomicInteger(0);
        static AtomicInteger poolNumb = new AtomicInteger(0);
        AtomicInteger threadId = new AtomicInteger(0);
        public AmigoThreadFactory(){
            poolId.set(poolNumb.incrementAndGet());
        }
        @Override
        public Thread newThread(Runnable r)
        {
            Thread t = new Thread(r);
            threadId.set(threadId.incrementAndGet());
            t.setPriority(5);
            t.setName(Thread.currentThread().getThreadGroup().getName() + "-pool-" + poolId + "-thread-" + threadId);
